...
class TrafficController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.reroute_cooldown = 15
        self.flow_refresh_interval = 30
        self.congested_link = None
        self.demo_mode = True
    ...
    def is_valid_path(self, path):
        ...
        for index in range(len(path) - 1):
            current = path[index]
            next_hop = path[index + 1]
            if self.adjacency.get(current, {}).get(next_hop) is None:
                return False

        return True

    def path_uses_link(self, path, link):
        if not path or not link or len(path) < 2:
            return False

        avoid_src, avoid_dst = link

        for index in range(len(path) - 1):
            if path[index] == avoid_src and path[index + 1] == avoid_dst:
                return True

        return False

    def get_alternate_path(self, src_dpid, dst_dpid, current_path, avoid_link):
        if not current_path or len(current_path) < 2:
            return None
        if not avoid_link:
            return None

        avoid_src, avoid_dst = avoid_link
        ...
        while queue:
            ...
            for neighbor in self.adjacency.get(current, {}):
                ...
                if neighbor == dst_dpid:
                    if next_path != current_path and self.is_valid_path(next_path):
                        return next_path
                    continue
    ...
    def install_path_flows(self, src_mac, dst_mac, path=None):
        ...
        for index, dpid in enumerate(path):
            ...
            self.add_flow(datapath, 10, match, actions, idle_timeout=0)
    ...
    def get_demo_congested_link(self):
        demo_src = 1
        demo_dst = 2
        demo_port = self.adjacency.get(demo_src, {}).get(demo_dst)

        if demo_port is None:
            return None

        return demo_src, demo_dst, demo_port
    ...
    def reroute_flows(self):
        ...
        now = time.time()
        rerouted = False
        congested_link = self.congested_link
        handled_pairs = set()

        if not congested_link:
            return

        for flow_key, flow_info in list(self.active_paths.items()):
            ...
            if flow_info.get('timestamp') is None:
                continue

            flow_pair = tuple(sorted((src_mac, dst_mac)))
            if flow_pair in handled_pairs:
                continue
            handled_pairs.add(flow_pair)
            ...
            if (not current_path or len(current_path) < 2 or
                    not self.is_valid_path(current_path)):
                continue

            if not self.path_uses_link(current_path, congested_link):
                continue

            new_path = self.get_alternate_path(
                src_dpid, dst_dpid, current_path, congested_link
            )
            if not new_path or len(new_path) < 2 or new_path == current_path:
                continue
            ...
            if not self.is_valid_path(reverse_path):
                continue
            ...
            if success and reverse:
                self.logger.info(
                    "REROUTING flow %s -> %s from path %s to path %s",
                    src_mac, dst_mac, current_path, new_path
                )
    ...
    def packet_in_handler(self, ev):
        ...
        # ---------------- LEARNING ----------------
        if dpid in self.access_ports and in_port in self.access_ports[dpid]:
    ...
    def port_stats_reply_handler(self, ev):
        ...
        for stat in body:
            ...
            if self.demo_mode:
                demo_link = self.get_demo_congested_link()
                if demo_link is not None:
                    demo_src, demo_dst, demo_port = demo_link
                    if ev.msg.datapath.id == demo_src and port == demo_port:
                        load += 1000000
                        self.logger.info("DEMO CONGESTION TRIGGERED")
            ...
            if (load > self.congestion_threshold and
                ...
                neighbor_dpid = None
                for candidate_dpid, candidate_port in self.adjacency.get(
                        ev.msg.datapath.id, {}).items():
                    if candidate_port == port:
                        neighbor_dpid = candidate_dpid
                        break

                if neighbor_dpid is not None:
                    self.congested_link = (ev.msg.datapath.id, neighbor_dpid)
